...
from collistion_utils import is_collision, interpolate_waypoints_radians, is_intersection_point_inside_triangle, get_link_poses, fine_interpolation
...
def get_mesh_data(mesh):
    # Attempt multiple common attribute names for vertices/faces/normals
    vertices = getattr(mesh, "v", None)
    if vertices is None:
        vertices = getattr(mesh, "vertices", None)
    if vertices is None:
        vertices = getattr(mesh, "points", None)
    ...
    normals = getattr(mesh, "vn", None)
    if normals is None:
        normals = getattr(mesh, "vertexnormals", None)
    if normals is None:
        normals = np.zeros_like(vertices) if vertices is not None else None

    if vertices is None or faces is None or normals is None:
        raise ValueError(f"Mesh {mesh} missing geometry attributes")

    return np.array(vertices), np.array(faces), np.array(normals)

##RMRC helper functions
def check_self_collision(robot, env_meshes=None, max_step=np.deg2rad(1)):
    q_matrix = interpolate_waypoints_radians([robot.q, robot.q], max_step_radians=max_step)
    ...
    if env_meshes:
        for mesh in env_meshes.values():
            try:
                v, f, n = get_mesh_data(mesh)
                vertices.append(v)
                faces.append(f)
                normals.append(n)
    ...
    return is_collision(robot, q_matrix, faces, vertices, normals, env=None, return_once_found=True)
...
for step_idx, (robot_idx, action, stl_name) in enumerate(global_sequence):
    ...
    for step in range(max_steps):
        ...
        if np.linalg.norm(pos_error) < tol_pos:
            ...
            break
        env_meshes_to_check = brick_meshes.copy()
        env_meshes_to_check["ground"] = env_mesh  # add main environment

        if check_self_collision(robot, env_meshes=env_meshes_to_check):
            print(f" {robot_name} collision detected, stopping motion!")
            break
        ...
        q_path = interpolate_waypoints_radians([robot.q, target_q], max_step_radians=np.deg2rad(1))
        for q_next in q_path:
            robot.q = q_next
            if check_self_collision(robot, env_meshes=env_meshes_to_check):
                print("Collision detected along path, aborting")
                break
            env.step(dt)
        T = robot.fkine(robot.q)
